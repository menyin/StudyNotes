...
*kafka的分区Leader和分区Follower，只有分区Leader提供读写服务，分区Follower只做备份提高可用性。  同理ES也是如此
...
？分区由谁决定。创建topic时，会指定分区数量。 分区数不指定则默认1
   具体设置经验规则详见csdn《Kafka分区数量设置详解》
   注意设置分区太多带来的问题：
   *可用性降低：分区设置多了，每个broker的分区越多，当一个broker挂掉，则其它broker上分区副本要选为leader就要更多的时间，此时主题topic是不可用的。 
   *系统资源限制：设置分区实际是为了高吞吐量，但是消费者或生产者也需要对应数量的线程或句柄去处理（以及内存占用），那么当分区太多，则可能导致消费者或生产者的线程或句柄超出系统负荷（内存也可能超出）。
...
？kafka什么情况下会丢失数据
  ...
  3、在kafka消息删除相关配置中，一些配置参数如果过小也会造成消息丢失的假象。 如log.retention.hours是设置topic消息保留时长，如果设置太小，消费者来不及消费消息就被删除了。

？kafka什么情况下会数据积压（Lag变大）

*kafka数据清理方式：数据删除、数据压缩
？kafka数据删除原理：
   过程：通过设置定期检测时间（log.retention.check.interval.ms默认5分钟）和消息保留时间（log.retention.hours|minutes|ms默认7天）将数据文件标记为deleted,
            然后通过file.delete.delay.ms设置定期时间删除带有deleted标记的数据。
   注意：1、这些参数可以在每个topic上进行设置
            2、上述删除过程是时间的保留策略，也可以基于日志大小的保留策略（log.retention.bytes）、基于日志起始偏移量保留策略（logStartOffset）
？kafka数据压缩原理：
  过程：将消息key相同的数据只保留一个最新的版本           
...
*概念(基本概念看csdn《Kafka基本概念》)
  ...
  offset：主题信息+消费者信息+分区信息+offset形成一组唯一标识存储在zookeeper里，用于记录每个或每组消费者的消费情况。早期版本存储在zookeeper，后面存储kafka的一个专门topic上
...
*自Kafka 0.9.0.0版本开始，Kafka已经将偏移量（offset）的管理从Zookeeper转移到内部主题（Internal Topic）__consumer_offsets中。
 在Kafka 2.8.0及更高版本中，Zookeeper中存储的元数据信息已经被彻底移除，Kafka完全采用了自己的元数据存储方式。  即KRaft模式
 注意：由于脱离zk的KRaft模式还是较新的功能，还未有市场的测试和检验。目前生产环境使用率是较低，另外一些EFAK、KafkaTool等管理工具也不支持，
          helm上的bitnami/kafka等仓库虽然用了KRaft模式，但没有相关管理工具的支持也是挺麻烦。 KafkaAssistant是一款付费软件但个人实测并不好用。
...
？将原本的单分区topic更改为多分区（通过KafkaInitialConfiguration更改），然后添加了100条helloword数据，然后在kafkaTool查看时发现，数据没有分配到其它新增分区，而是在原本分区上。！应该是100条样本数太少了，测试个1w条就可以了。 
  有上述也可以看出kafka的分区能存储多少消息数据，主要是看磁盘和消息本身大小以及消息保留策略。并没有特定参数去规定能存储多少消息数据
...
？linux下怎么查看通过helm安装的kafka等应用的安装目录和配置文件目录？
   进入到容器内，尝试查找 /opt、/etc 或 /var 目录下的相关文件和目录。如helm安装的kafka在/opt底下可以查看相关文件

？

？在springboot项目中集成@KafkaListener进行监听消费时，虽然已经配置了自动提交和延迟提交秒数，但收到一条并没有提交offset。

**kafka管理工具
 kafka 管理工具查看消费者的offset上有个指标lag，他是什么意思？正负代表什么意思？
 先看单词意思：滞后，（时间上的）间隔;
 ...
 总之：lag=end-offset   其中end是最后一条消息的索引


*kafka缺点：
  过度依赖带宽和磁盘性能
  生产和消费时批量提交缓冲提交会有比较大延迟
  由于reblance或应答模式设置问题会造成数据丢失
  依赖zookeeper，所以zk的leader选举时不可用，kafka也会不可用
...
*kafka、redis、elasticsearch都有主分片和副本分片机制。那么副本分片都有复制同步主分片的作用，而副本分片只有ES才能进行读操作，kafka和redis的副本只作为备份做高可用？
...
*await和wait的区别。 详见csdn《Java并发：await/wait的区别》
 lock体系会比synchronized更灵活，例如：用多个线程用wait()阻塞等待，用notify()不能唤醒指定线程。 而如果用多个线程用await()阻塞等待，当lock是公平锁时，则通过signal()可以唤醒等待队列首个线程。即能实现公平锁
...
***JUC AQS核心三板斧：自旋、CAS、LockSuport。详见腾讯课堂图灵学院杨过老师并发编程课
   ...
   以reentranlock为例，AQS的整体大概流程为：
    1、多个线程调用reentranlock.lock()竞争锁，原理是定义一个全局的state变量，线程t0通过CAS原子操作state成功代表获取锁成功。
    2、reentranlock用一个变量（比如叫：lockHolder）记录先获取锁的线程，用一个队列（比如叫：waiters）按顺序记录未获取锁的线程（如果设置了公平锁则是有序队列否则无序）。
    ...
    4、未获取锁的线程，进入自旋，实际就是循环while(true){先进行尝试获取锁，未果则用Locksuport.park()阻塞//等待持锁线程释放锁唤醒。
        注意：如果设置了公平锁则是获取有序队列首个线程唤醒它去获取锁，即LockSupport.unpark(t1);
...
*ACID 即原子性、一致性、隔离性及持久性，是传统事务特性，一个事务要满足这四个特性
 CAP 一致性、可用性、分区容错性，是分布式事务下只能这三者中的两个。并且这里的一致性属于强一致性。
     ...
     可理解场景：在zookeeper集群中，因为网络原因导致集群分为两个区，但是两个区还可以进行读（分区容错性）。而每个分区如果不能重新选举leader（偶数节点导致投票未过半），那么有个分区就没leader，就无法做增删改（只有leader才能做事务操作），即没有可用性。   但即使分区了，通过zab协议保证两个分区只选出了一个合法leader，然后广播同步到两个分区的follower，从而保证数据一致性。   注意投票选举中投票过半是指整个集群节点总数而不是网络分区后单分区的节点总数。
   ...
   *实践中相关问题：
       ...
       *seata demo中/test2中，account服务余额不足时，account本地事务失败（抛出异常），order服务事务回滚了，但product的事务却提交了（库存变了），也就是没有全局事务回滚。
        ...
        解决：根本原因在于orderServer用feign客户端请求微服务未进行xid携带，productServer和accountServer获取不到xid。
             jeecgdemo里用的是seata-spring-boot-starter并没有xid携带功能。而spring-cloud-starter-alibaba-seata是集成xid携带功能(放在请求header的tx_xid)。或者网上有“添加一个SeataFilter.java，然后去绑定xid”方案
             详见csdn《SpringCloud-Alibaba之Seata入门以及踩坑》、blogs《spring boot:shardingsphere+druid整合seata分布式事务(spring boot 2.3.3) 》
        总结！！！：上述xid问题，实际上seata本身是一个多库分布式事务的解决方案，但是微服务涉及到多服务+多库，seata要做多服务多库事务则必须让xid在多服务之间传递。所以分库分表后尽量在单服务里用seata做分布式事务
             Shardingsphere+seata整合原理过程详见shardingsphere官网《Apache ShardingSphere整合Seata AT分布式事务》     https://shardingsphere.apache.org/blog/cn/material/seata/
