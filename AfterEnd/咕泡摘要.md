 ...
 **消息队列的对比：
  有broker的：即有中间存储服务器的，就是所谓的中间件
   RocketMQ是ActiveMQ的升级版
   Kafka速度是最快的，但是可能丢包，可以用于传输相对不重要的数据。 传输速度快原理：创建一次连接，多个数据打成一个包进行传输。
   RibbitMQ轻量级MQ，上限取决于机器内存，
 ...
 * seata框架，阿里的分布式框架seata后期可以了解下，19年1月开始开源的项目，还不稳定，待研究。
   详见blibli的收藏的马士兵的课程
   总体逻辑：两个独立的局部事务tx1、tx2注册到一个全局事务，会有保留全局事务编号XID和局部事务BranchID
